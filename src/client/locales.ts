...
export const zh = {
  ...
  'select.all': '全选本页',
  'select.row': '选择 {name}',
  'select.count': '已选择 {count} 条',
  'select.clear': '取消选择',
  'select.delete': '批量删除',

  'batch.kicker': '破坏性批量操作',
  'batch.title': '批量删除知识条目',
  'batch.close': '关闭批量删除对话框',
  'batch.body': '这会把选中的 {count} 条知识条目从当前书架逐条移除。批量删除不是事务：中途失败的条目会被跳过并在结果里列出，已删除的不会回滚。',
  'batch.list': '将删除以下条目',
  'batch.cascade': '同时删除这些条目的相关 statement',
  'batch.confirm.label': '键入 {count} 以确认',
  'batch.cancel': '取消',
  'batch.submit': '删除所选条目',
  'batch.done': '已删除 {count} 条知识条目：statement 删除 {deleted} 条、保留 {retained} 条。',
  'batch.done.issues': '另有 {missing} 条已不存在，{failed} 条删除失败。',
  'batch.failed': '批量删除失败。',
...
export const en = {
  ...
  'select.all': 'Select every record on this page',
  'select.row': 'Select {name}',
  'select.count': '{count} selected',
  'select.clear': 'Clear selection',
  'select.delete': 'Delete selected',

  'batch.kicker': 'DESTRUCTIVE BULK ACTION',
  'batch.title': 'Delete selected entries',
  'batch.close': 'Close bulk delete dialog',
  'batch.body': 'This removes the {count} selected entries from this shelf, one at a time. A batch is not a transaction: an entry that fails is skipped and listed in the result, and entries already removed are not rolled back.',
  'batch.list': 'Entries to delete',
  'batch.cascade': 'Also delete the statements related to these entries',
  'batch.confirm.label': 'Type {count} to confirm',
  'batch.cancel': 'Cancel',
  'batch.submit': 'Delete selected',
  'batch.done': 'Deleted {count} record(s): {deleted} statement(s) removed, {retained} retained.',
  'batch.done.issues': '{missing} were already gone and {failed} failed.',
  'batch.failed': 'Bulk delete failed.',
